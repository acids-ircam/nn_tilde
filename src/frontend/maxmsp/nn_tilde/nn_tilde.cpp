...
path nn::to_model_path(std::string model_path) {
  try {
...
void nn::update_model(const std::string &model) {
  load_model(model);
}

void nn::load_model(const std::string& model_path) {
  ...
  if (m_model_in != n_inlets) {
    cwarn << "nn~ has been initialised with " << n_inlets << " inputs, but current model has " << m_model_in << endl;
  }
  if (m_model_out != n_outlets) {
    cwarn << "nn~ has been initialised with " << n_outlets << " outputs, but current model has " << m_model_out << endl;
  }
